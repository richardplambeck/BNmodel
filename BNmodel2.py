...
hplanck = 6.626e-27        # planck's constant, erg s
...
# originally this routine calculated the EM across just one quadrant, but now the calculation is done
#   in all 4 quadrants to allow for source that is inclined relative to the line of sight, and for
#   future upgrade to include velocity information for recombination line profiles
# xAU,yAU,zAU are grid in sky coordinates, with z along line of sight; image in the (x,y) plane
# for each cell [xAU,yAU,zAU] the routine computes the corresponding position [xdAU,ydAU,zAU] in
#   the disk (object) coordinates, which are rotated by angle theta_i around the x-axis (that is,
#   the yd axis is not in the plane of the sky, and the zd axis is not parallel to the line of sight
# phi is the halfwidth of the biconical ionized regions (phi_deg = 90 for a spherical wind) relative
#   to the zd axis
# -----------------------------------------------------------------------------------------------------------#
def EMcalc( model, pklfile ):
    phi_rad = numpy.radians( model["phi_deg"] )	            # halfangle of biconical ionized wind relative to zd
    cosi = math.cos(numpy.radians( model["thetai_deg"] ))   # cosine of inclination angle
    sini = math.sin(numpy.radians( model["thetai_deg"] ))   # sine of inclination angle
    print "cosi = %.3f, sini = %.3f" % (cosi,sini)
    gridAU = numpy.arange( -1.*model["rmax_AU"], model["rmax_AU"] + model["incr_AU"]/2., model["incr_AU"] )
      # grid has an odd number of pixels, centered at 0
    print gridAU
    nn = len( gridAU )
    EMimg = numpy.zeros( (nn,nn) )			    # emission measure image in (x,y) sky coord frame
    for i in range( 0, nn ):
      xdAU = gridAU[i]                                      # disk coord = sky coord for x
      for j in range( 0, nn ):
        yAU = gridAU[j]
        for k in range( 0, nn ):
          zAU = gridAU[k]
          ydAU = yAU*cosi - zAU*sini                        # disk coord ydAU for this yAU,zAU
          zdAU = yAU*sini + zAU*cosi                        # disk coord zdAU for this yAU,zAU
          r = math.sqrt( pow(xdAU,2) + pow(ydAU,2) + pow(zdAU,2) ) 
          if (r < model["rmax_AU"]) and (math.atan2(math.sqrt(pow(xdAU,2) + pow(zdAU,2)), abs(ydAU) ) < phi_rad) :
...
# -----------------------------------------------------------------------------------------------------------#
# given EM image (perhaps just 1 quadrant), return continuum and Ha recomb line brightness temperature images
...
def Tff ( fGHz, Te, fwhm, EMimg ) :
    ...
    print "peak TcontImage = %.0f" % numpy.amax(TcontImage)
...
def fit2dgaussian(x,y,img):
    ...
    print "len(xdata) = %d" % (len(xdata))
    guesspeak = numpy.amax(img)
	# note: max of convolved image is >> max of original image
...
# this routine is needed because directly fitting gaussian to bicone can give nonsensical answers
# -----------------------------------------------------------------------------------------------------------#
...
#   if desired, convolve model with synthesized beam and compare with observed image
# old is temporary fix for older version where I computed only 1 quadrant
# -----------------------------------------------------------------------------------------------------------#
def plotTb (pklfile, fGHz, type='continuum', rotdeg=70., obsImage=None, Old=False ) :
    try :
      fin = open( pklfile, "rb" )
  ...
  # mirror to 4 quadrants, create x and y axes in arcsec
    if Old :
      ...
      if type == 'line' :
        ...
        img1 = TcontImg
    x_arcsec = model["gridAU"]/model["dist_AU"] 
    y_arcsec = model["gridAU"]/model["dist_AU"] 
    x,y = numpy.meshgrid( x_arcsec, y_arcsec )
  
  # Gaussian fit to source 
    bmaj,bmin,img2 = fitConv2dgaussian(x, y, img1)
      # ignore img2, which is img1 convolved with .02 arcsec Gaussian
  ...
  # rotate both model image and gaussian fit by rotdeg
    ...
    ax1.contour( x, y, img5, [0.5], colors='white'  )     # plot Gaussian fit FWHM 
    fig.colorbar( cs )
    ...
    ax1.annotate( "continuum", (.04,.91), xycoords='axes fraction', color='white')
